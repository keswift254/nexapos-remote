...
class _RegistrationGateway extends PlatformOnboardingGateway {
  int calls = 0;
  ...
  Future<DeviceRegistration> registerDevice({
    ...
    calls++;
    return const DeviceRegistration('registered-key');
  }
}
...
void main() {
  ...
  Future<void> pumpApp(
    ...
    PaystackCredentials credentials = _testCredentials,
  }) async {
      ...
      ProviderScope(
        overrides: [
          ...
          licenseGatewayProvider.overrideWith(
            (ref) => LicenseGateway(licenseClient),
          ),
          currentPaymentCredentialsProvider.overrideWith(
            (ref) async => credentials,
          ),
          if (onboarding != null)
            platformOnboardingGatewayProvider.overrideWithValue(onboarding),
        ],
  ...
  testWidgets(
    'fresh activation opens setup while registering the primary device',
    (tester) async {
      final onboarding = _RegistrationGateway();
      await pumpApp(
        tester,
        onboarding: onboarding,
        credentials: const PaystackCredentials(
          baseUrl: '',
          apiKey: '',
          currency: 'KES',
          defaultEmail: '',
        ...
        licenseClient: MockClient(
          (_) async => http.Response(
            jsonEncode({'success': true, 'activation_token': 'b' * 64}),
            200,
          ),
        ),
      );
      await tester.enterText(
        find.widgetWithText(TextFormField, 'License key'),
        'FRESH12345',
      );
  ...
  testWidgets('a valid code activates the app and reveals setup', (
    tester,
  ) async {
    await pumpApp(
      tester,
      licenseClient: MockClient((request) async {
        ...
        return http.Response(
          jsonEncode({'success': true, 'activation_token': 'a' * 64}),
          200,
        );
      }),
    );
  ...
  testWidgets(
    'a rejected code shows the backend message and stays on the gate',
    (tester) async {
      await pumpApp(
        tester,
        licenseClient: MockClient((request) async {
          return http.Response(
            jsonEncode({'success': false, 'message': 'Invalid license key.'}),
            422,
          );
        }),
      );
  ...
  testWidgets('an unreachable license server shows a clear offline message', (
    tester,
  ) async {
    await pumpApp(
      tester,
